...
import { mcGachaDataPath, pluginName, pluginVer } from '../data/system/pluginConstants.js'
import fs from 'fs'
...
export default class mcGachaData {
  ...
  async show() {
    let msg = this.e.msg
      .replace(/#| /g, '')
      .replace(/记录|唤取|分析|池/g, '')

    const tokenData = await getToken(this.e.user_id)
    kuroLogger.debug(`QQ ${this.e.user_id} 的 tokenData: ${JSON.stringify(tokenData)}`)


    // 尝试获取用户的绑定信息和抽卡链接
    let user = new userConfig()
    let gameId = 3
    let gameUid = (await user.getCurGameUidLocal(this.e.user_id, gameId))?.gameUid
    let gachaLink = await user.getMcGachaDataLink(this.e.user_id, gameUid)
    kuroLogger.debug(`QQ ${this.e.user_id} 的游戏 uid: ${gameUid}, 抽卡链接: ${gachaLink}`)
    
    // 先检查本地是否既没有抽卡记录也没有抽卡链接, 如果没有就提示获取抽卡记录
    if(!(await this.exist(this.e.user_id, gameUid)) && !gachaLink) {
      this.e.reply(`QQ ${this.e.user_id} 的游戏 uid ${gameUid} 暂未获取抽卡记录\n请发送 #鸣潮抽卡记录帮助 以获取记录`)
      return
    }

    // 然后检查是否是通过链接上传的信息, 如果是链接上传就更新再生成, 如果是本地上传就直接生成
    if(gachaLink) {
      this.e.reply(`QQ ${this.e.user_id} 的游戏 uid ${gameUid} 本地存在抽卡链接, 尝试更新抽卡记录...`)
      let gachaRecord = await this.get(gachaLink, this.e.user_id)
      if (typeof gachaRecord === 'string') { // 如果记录更新失败, 进行提示; 如果更新成功, 不提示直接进入生成
        this.e.reply(`QQ ${this.e.user_id} 的游戏 uid ${gameUid} 更新抽卡记录失败: \n${failedReason}\n将展示历史抽卡记录`)
      } else { // 更新成功, 存入本地
        await this.update(this.e.user_id, gachaRecord)
      }
    } else {
      this.e.reply(`QQ ${this.e.user_id} 的游戏 uid ${gameUid} 通过本地上传的抽卡记录, 将展示历史抽卡记录`)
    }
 ...
 */
  async get(link, qq) {
    ...
    // 检查链接及参数
    if (!link.startsWith('https://aki-gm-resources.aki-game.com/aki/gacha/index.html#/record?')) {
    ...
    if (!serverId || !playerId || !recordId || !cardPoolId) {
      kuroLogger.debug(`QQ ${qq} 的抽卡链接 ${link} 参数缺失, serverId: ${serverId}, playerId: ${playerId}, recordId: ${recordId}, cardPoolId: ${cardPoolId}`)
      return '抽卡链接参数缺失'}
    if (!serverId.match(/^[a-z0-9]{32}$/) || !playerId.match(/^\d{9}$/) || !recordId.match(/^[a-z0-9]{32}$/) || !cardPoolId.match(/^[a-z0-9]{32}$/)) {
      kuroLogger.debug(`QQ ${qq} 的抽卡链接 ${link} 参数格式错误, serverId: ${serverId}, playerId: ${playerId}, recordId: ${recordId}, cardPoolId: ${cardPoolId}`)
      return '抽卡链接参数格式错误'}
    
    // 发送请求
    let kuroapi = new kuroApi(qq)
    let rsp_mcGachaRecord = await kuroapi.mcGachaRecord(0, { cardPoolId, cardPoolType: 7, playerId, recordId, serverId })
    if (typeof rsp_mcGachaRecord === 'string') {
      kuroLogger.debug(`QQ ${qq} 的抽卡链接 ${link} 获取失败: ${rsp_mcGachaRecord}`)
      return `QQ ${qq} 的抽卡链接 ${link} 获取失败: \n${rsp_mcGachaRecord}`}

    // 完成测试, 保存记录到本地
    let user = new userConfig()
    kuroLogger.debug(`QQ ${qq} 的抽卡链接 ${link} 获取成功, 保存链接及游戏 uid...`)
    ...
    for (let curCardPoolToFetch = 1; curCardPoolToFetch <= 6; curCardPoolToFetch++) {
      let rsp_mcGachaRecord = await kuroapi.mcGachaRecord(0, { cardPoolId, cardPoolType: curCardPoolToFetch, playerId, recordId, serverId })
      if (typeof rsp_mcGachaRecord === 'string') // 失败重试一次
        rsp_mcGachaRecord = await kuroapi.mcGachaRecord(0, { cardPoolId, cardPoolType: curCardPoolToFetch, playerId, recordId, serverId })
      if (typeof rsp_mcGachaRecord === 'string') rsp_mcGachaRecord = { code: -1, message: rsp_mcGachaRecord, data: [] }
      kuroLogger.debug(`QQ ${qq} 的抽卡链接 ${link} 获取卡池 ${curCardPoolToFetch} 的抽卡记录结果: ${JSON.stringify(rsp_mcGachaRecord)}`)
      gachaDataJson.gachaData[curCardPoolToFetch] = rsp_mcGachaRecord
    }

    kuroLogger.debug(`QQ ${qq} 的抽卡链接 ${link} 获取抽卡记录完成: ${JSON.stringify(gachaDataJson)}`)
 ...
 */
  async update(qq, gachaDataJson) {
    kuroLogger.debug(`尝试保存 QQ ${qq} 的抽卡记录 ${JSON.stringify(gachaDataJson)} 到本地...`)
    // 检查 gachaDataJson 版本是否兼容
    if (!gachaDataJson.version || gachaDataJson.version !== 1) {
      kuroLogger.debug(`QQ ${qq} 的抽卡记录版本 ${gachaDataJson?.version} 不兼容: ${gachaDataJson.version}`)
      return '抽卡记录版本不兼容'}
    ...
    kuroLogger.debug(`准备转换 QQ ${qq} 的抽卡记录到 UIGF 格式: ${JSON.stringify(gachaDataInUniform)}`)
    ...
    for (let i = 1; i <= 6; i++) {
      ...
      for (let j = 0; j < gachaData.length; j++) {
        ...
        let id = Date.parse(data.time) + i.toString() + (timeCount[data.time]--).toString().padStart(2, '0');
        
        kuroLogger.debug(`QQ ${qq} 的抽卡记录 ${i} 在时间 ${data.time} 下的物品 ${JSON.stringify(data)} 是第 ${timeCount[data.time] + 1} 个, 其 id 为 ${id}`)
    ...
    kuroLogger.debug(`QQ ${qq} 的 UIGF 抽卡记录转换完成: ${JSON.stringify(gachaDataInUniform)}`)
