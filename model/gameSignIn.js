                        ...
                        try {
                            ...
                            if (rsp_initSignIn.code === 200) {
                                if (rsp_initSignIn.data.sigIn) {
                                    //如果今天已经签到
                                    doPnsSignInRet += `      今日已签, 本月签${rsp_initSignIn.data.sigInNum}天` + (rsp_initSignIn.data.omissionNnm !== 0 ? `, 漏${rsp_initSignIn.data.omissionNnm}天` : '') + `\n`
                                } else {
                                    //
                                    ...
                                    formData.append('serverId', data.serverId)
                                    formData.append('roleId', data.roleId)
                                    formData.append('reqMonth', (new Date().getMonth() + 1).toString().padStart(2, '0')) // 去当前月份, 形如 07
                                    ...
                                    try {
                                        ...
                                        if (!response_signIn.ok) {
                                            doPnsSignInRet += `      ${response_signIn.status}, 本月签${rsp_initSignIn.data.sigInNum}天` + (rsp_initSignIn.data.omissionNnm !== 0 ? `, 漏${rsp_initSignIn.data.omissionNnm}天` : '') + `\n`
                                        }
                                        ...
                                        if (rsp_signIn.code === 200) {
                                            //签到成功
                                            doPnsSignInRet += `      签到成功, 本月签${rsp_initSignIn.data.sigInNum + 1}天` + (rsp_initSignIn.data.omissionNnm !== 0 ? `, 漏${rsp_initSignIn.data.omissionNnm}天` : '') + `\n`
                                        } else {
                                            doPnsSignInRet += `      ${response_signIn.msg}, 本月签${rsp_initSignIn.data.sigInNum}天` + (rsp_initSignIn.data.omissionNnm !== 0 ? `, 漏${rsp_initSignIn.data.omissionNnm}天` : '') + `\n`
                                        }
                                    } catch (error) {
                                        doPnsSignInRet += `      请求出错: ${JSON.stringify(error)}, 本月签${rsp_initSignIn.data.sigInNum}天` + (rsp_initSignIn.data.omissionNnm !== 0 ? `, 漏${rsp_initSignIn.data.omissionNnm}天` : '') + `\n`
                                    }
