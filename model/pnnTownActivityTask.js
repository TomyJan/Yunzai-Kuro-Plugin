...
export default class PnnTownActivity {
  ...
  async pnnTownSendCode () {
    ...
    this.e.reply('验证码发送成功, 请再次发送手机号和验证码, 以完成登录 \n格式: 帕尼尼小镇18888888888,验证码114514')
    return true
  }

  async pnnTownDo () {
    let msg = this.e.msg
      .replace(/帕尼尼小镇|：|:|一键|活动|任务|登录|登陆|账号|手机号|验证码/g, '')
      .replace(/,|，/g, ',')
      .replace(/#| /g, '')
      .split(',')
    ...
    if (!rspPnsTownLogin.data.token || rspPnsTownLogin.data.token == '') {
      this.e.reply('登录失败: 未知错误, 取消任务. 活动页返回: \n' + JSON.stringify(rspPnsTownLogin))
      return false
    }
    ...
    if (typeof rspPnsTownUserInfo == 'string') {
      // 不是 json, 即返回报错
      this.e.reply(`查询任务进度失败, 取消任务. 活动页返回: \n${rspPnsTownUserInfo}`)
      return false
    }

    if (rspPnsTownUserInfo?.data?.num == undefined || rspPnsTownUserInfo?.data?.chance_num == undefined) {
      this.e.reply('查询任务进度失败: 未知错误, 取消任务. 活动页返回: \n' + JSON.stringify(rspPnsTownUserInfo))
      return false
    }
    let taskNum = rspPnsTownUserInfo?.data?.num
    let chanceRemain = rspPnsTownUserInfo?.data?.chance_num
    this.e.reply(`查询任务进度成功, 当前${taskNum > 0 ? (taskNum == 20 ? '已完成所有任务, 跳过任务' : `已完成 ${taskNum} 个任务`) : '未完成任何任务'}`)
    ...
    if (continueTask) {
      ...
      for (let i = 1; i < 19; i++) {
        ...
        if (typeof rsp == 'string') {
          // 不是 json, 日志报错就行
          kuroLogger.warn(`用户 ${this.e.user_id} 手机号 ${msg[0]} 帕尼尼小镇在第 ${i} 个蛋时出错, 接口返回: ${JSON.stringify(rsp)} , 重试一次`)
          rsp = await pnnAct.pnnTownActCollectEgg(token, i)
          kuroLogger.debug('rsp_pnnTownActCollectEgg_retry:', JSON.stringify(rsp))
          if (typeof rsp == 'string') {
            kuroLogger.error(`用户 ${this.e.user_id} 手机号 ${msg[0]} 帕尼尼小镇在第 ${i} 个蛋时出错, 接口返回: ${JSON.stringify(rsp)}, 重试失败, 跳过这个蛋`)
            this.e.reply(`任务 ${i} 出错, 活动页返回: \n ${JSON.stringify(rsp)}`)
          }
        }
      ...
      if (typeof rspShare == 'string') {
        // 不是 json, 重试一次
        kuroLogger.warn(`用户 ${this.e.user_id} 手机号 ${msg[0]} 帕尼尼小镇分享任务出错, 接口返回: ${JSON.stringify(rspShare)} , 重试一次`)
        rspShare = await pnnAct.pnnTownActShare(token)
        kuroLogger.debug('rsp_pnnTownActShare_retry:', JSON.stringify(rspShare))
        if (typeof rspShare == 'string') {
          kuroLogger.error(`用户 ${this.e.user_id} 手机号 ${msg[0]} 帕尼尼小镇分享任务出错, 接口返回: ${JSON.stringify(rspShare)}, 重试失败, 跳过这个任务`)
    ...
    if (typeof rspPnsTownUserInfo == 'string') {
      // 不是 json, 即返回报错
      this.e.reply(`查询任务进度失败, 取消抽奖. 活动页返回: \n${rspPnsTownUserInfo}`)
    } else {
      chanceRemain = rspPnsTownUserInfo?.data?.chance_num
      continueTask = chanceRemain > 0
      this.e.reply(`当前${continueTask ? `剩余 ${chanceRemain} 次抽奖机会, 开始抽奖` : '抽奖机会已用完, 跳过抽奖, 准备查询奖品'}`)
    }

    if (continueTask) {
      for (let i = 1; i < chanceRemain + 4; i++) { // 多抽几次, 防止出错
        let type = 1
        if (chanceRemain > 10 && i < 10) { // 如果剩余十次以上, 先来一发十连
          type = 2
          i = 10
        }
        ...
        if (typeof rsp == 'string') {
          // 不是 json, 日志报错就行
          kuroLogger.warn(`用户 ${this.e.user_id} 手机号 ${msg[0]} 帕尼尼小镇第 ${i}/${chanceRemain + 4} 次抽奖时出错, 接口返回: ${JSON.stringify(rsp)}`)
        }
        await new Promise(resolve => setTimeout(resolve, Math.floor(Math.random() * 100 + 100)))
      }
    ...
    for (let i = 0; i < rspPrize.data.length; i++) {
      prizeList.push({ name: rspPrize.data[i].item_name, type: rspPrize.data[i].type_ename, code: rspPrize.data[i].code })
    }
    ...
    if (otherList.length > 0) {
      priceMsgs.push('这就是首席的实力! 你居然抽到了蚊子腿以外的东西! 快看看有什么吧~')
