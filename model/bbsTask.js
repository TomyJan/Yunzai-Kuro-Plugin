...
export async function doBBSDailyTask(uin, kuro_uid) {
  ...
  do {
    ...
    if (tryTimes++ >= 3 || typeof rsp_signIn !== 'string' || rsp_signIn === '您已签到' || rsp_signIn === 'token 失效') tryAgain = false

    if (!tryAgain) { // 最后一次尝试了那就处理返回值吧
      if (typeof rsp_signIn !== 'string' && rsp_signIn.code === 200) doBBSDailyTaskRet += '签到成功\n'
      else if (rsp_signIn === '您已签到') doBBSDailyTaskRet += '今日已签\n'
      // 顺便在这里直接处理 token 失效
      else if (rsp_signIn === 'token 失效') return `账号 ${kuro_uid}: \ntoken 失效\n`
  ...
  if(rsp_list) { // 获取到帖子就浏览点赞, 获取不到上面已经返回错误了
    //开始尝试 6 次浏览帖子
    tryAgain = true
    tryTimes = 0
    let succCount = 0
    do {
      let rsp_getPostDetail = await kuroapi.getPostDetail(kuro_uid, {postId: rsp_list.data.postList[tryTimes].postId})
    ...
    succCount = 0
    do {
      let rsp_like = await kuroapi.like(kuro_uid, {forumId: 2, gameId: 2,likeType: 1, operateType: 1,postCommentId: 0, postCommentReplyId: 0, postId: rsp_list.data.postList[tryTimes].postId, postType: rsp_list.data.postList[tryTimes].postType, toUserId: rsp_list.data.postList[tryTimes].userId})
